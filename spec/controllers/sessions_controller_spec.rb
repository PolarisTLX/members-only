...
RSpec.describe SessionsController, type: :controller do
    ...
    describe 'when user info is valid' do
      describe 'User does not want to be remembered' do
        it 'logs in user and redirects to user page' do
          post :create, params: { session: { username: user.username, password: 'password', remember_me: '0' } }
      ...
      describe 'User wants to be remembered' do
        it 'logs in user, remembers user, and redirects to user page' do
          post :create, params: { session: { username: user.username, password: 'password', remember_me: '1' } }
          expect(logged_in?).to be true
  ...
  describe 'DELETE #destroy' do
    it 'logs the user out' do
      delete :destroy, params: { session: { username: user.username, password: 'password' } }
      expect(logged_in?).to be false
